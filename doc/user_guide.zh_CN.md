...
当临时目录通常应该自动清理时，使用 `TempDir`。目录会立即创建，并在 guard drop 时递归
...
`Drop` 中的清理是 best-effort。如果删除失败，`TempDir` 会通过 `log` 门面记录 warning，
不会 panic。

## 临时文件

当你既需要唯一路径，又需要一个已经打开的文件句柄时，使用 `TempFile`。除非调用 `keep`
...
`TempFile::persist` 会先关闭文件句柄，再把临时文件移动到最终路径。如果目标文件不能被外部
观察到“只写了一半”，优先使用 `Files::atomic_write`。

## Atomic Write

`Files::atomic_write` 会在同一父目录下写入临时文件，flush 并 sync 这个临时文件，替换目标，
并在支持的平台上 sync 父目录。
...
当内容生成逻辑需要直接使用临时文件句柄时，使用 `Files::atomic_write_with`。
...
`Filenames` 提供不访问文件系统的 lexical helper。返回文件名数据的公开方法返回 UTF-8
...
`TempFile` 和 `TempDir` 创建的是本地文件系统条目；如果创建失败，会返回操作系统错误。
