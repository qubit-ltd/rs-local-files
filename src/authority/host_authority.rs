...
impl HostAuthority {
    ...
    pub(crate) fn bind_current(symlink_policy: LocalSymlinkPolicy) -> LocalResult<Self> {
        let diagnostic_cwd = env::current_dir()
            .map_err(|error| LocalFileError::from_io(LocalFileOperation::BindPath, None, None, error))?;
    ...
    pub(crate) fn resolve(&self, path: &Path) -> LocalResult<AuthorityPath> {
        if path.is_absolute() {
            ...
            let relative = symlink_resolver::resolve(&namespace, &relative, self.symlink_policy)?;
            Ok(AuthorityPath::Host(HostPath::Absolute(root.join(relative.as_path()))))
        } else {
            let path = RelativePath::parse(path)?;
            symlink_resolver::resolve(&self.cwd, &path, self.symlink_policy)
                .map_err(|error| error.with_path(self.diagnostic_cwd.join(path.as_path())))
    ...
    pub(crate) fn resolve_metadata(&self, path: &Path) -> LocalResult<AuthorityPath> {
        if path.is_absolute() {
            let (root, relative) = split_absolute_path(path)?;
            let namespace = NamespaceHandle::open_root(&root)?;
            let relative = symlink_resolver::resolve_parent(&namespace, &relative, self.symlink_policy)?;
            Ok(AuthorityPath::Host(HostPath::Absolute(root.join(relative.as_path()))))
        } else {
            let path = RelativePath::parse(path)?;
            symlink_resolver::resolve_parent(&self.cwd, &path, self.symlink_policy)
                .map_err(|error| error.with_path(self.diagnostic_cwd.join(path.as_path())))
                .map(HostPath::BoundCwd)
                .map(AuthorityPath::Host)
        }
    ...
    pub(crate) fn rename(&self, source: &AuthorityPath, target: &AuthorityPath, overwrite: bool) -> LocalResult<()> {
        ...
        source
            .namespace
            .handle()
            .rename_to(&source.relative, target.namespace.handle(), &target.relative, overwrite)
    }
    ...
    pub(crate) fn create_staged_file(&self, target: &AuthorityPath) -> LocalResult<StagedFile> {
        let target = self.namespace_path(target)?;
        target.namespace.handle().create_staged_file(&target.relative)
    }
    ...
    fn namespace_path<'authority>(&'authority self, path: &AuthorityPath) -> LocalResult<NamespacePath<'authority>> {
        match host_path(path)? {
            ...
            HostPath::Absolute(path) => {
                let (root, relative) = split_absolute_path(path)?;
                Ok(NamespacePath {
                    namespace: NamespaceSelection::NativeRoot(NamespaceHandle::open_root(&root)?),
                    relative,
                })
            }
        }
...
fn host_path(path: &AuthorityPath) -> LocalResult<&HostPath> {
    match path {
        ...
        AuthorityPath::Rooted(_) => Err(authority_mismatch(path.diagnostic_path())),
    }
}
