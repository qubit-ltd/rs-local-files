...
fn test_local_file_writer_preserves_logical_diagnostic_path() {
    ...
    let writer = LocalFileSystem::host()
        ...
        .open_writer_with_options(&logical, &LocalWriteOptions::new(LocalWriteMode::Append))
        .expect("append writer should open through the symlink");

    assert_eq!(Some(logical.as_path()), writer.diagnostic_path());
}
...
fn test_local_file_writer_append_abort_reports_aborted_and_published_states() {
    ...
    let mut untouched = LocalFileSystem::host()
        .expect("Host filesystem should open")
        .open_writer_with_options(&target, &LocalWriteOptions::new(LocalWriteMode::Append))
        .expect("append writer should open");
    ...
    let published_outcome = published.abort().expect("append abort should flush");
...
fn test_local_file_writer_returns_retryable_writer_before_publication() {
    ...
    fs::remove_file(&target).expect("existing destination should be removed before commit");
...
fn test_local_file_writer_abort_reports_missing_host_staging_file() {
    ...
    fs::remove_file(&staging).expect("external actor should remove staging file");

    let error = writer
        .abort()
        .expect_err("missing host staging file must report explicit cleanup failure");
...
fn test_local_file_writer_rooted_prepublication_failure_retains_writer() {
    ...
    let directory = tempdir().expect("temporary directory should be created");
    let target = directory.path().join("payload");
    fs::write(&target, b"existing").expect("existing rooted payload should be written");
    let rooted = LocalFileSystem::rooted(directory.path()).expect("rooted filesystem should open");
    ...
    fs::remove_file(&target).expect("inspected rooted destination should be removed");
...
fn test_local_file_writer_exercises_atomic_install_fault_boundaries() {
    ...
    for (fault, existing, fails) in [
        ...
        ("atomic-destination-would-block", true, true),
        ...
        run_host_writer_fault(TEST_NAME, fault, || {
            ...
            let target = directory.path().join("payload");
            if existing {
                fs::write(&target, b"existing").expect("replacement target should be written");
            }
