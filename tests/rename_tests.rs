...
fn test_rename_moves_file() {
    ...
    qubit_local_files::rename::move_path(&source, &target).expect("the file should move");
...
fn test_rename_without_replacing_preserves_existing_destination() {
    ...
    let error = qubit_local_files::rename::move_path_without_replacing(&source, &target)
