...
use qubit_local_files::{
    ...
    LocalFileSystem, LocalFileSystemScope, LocalListOptions, LocalReadOptions, LocalRenameOptions,
...
fn test_rooted_local_file_system_copy_and_rename_report_durability() {
    let directory = tempdir().expect("temporary directory should be created");
    let rooted = LocalFileSystem::rooted(directory.path()).expect("root authority should open");
    let implements_durability = rooted.capabilities().implements_durable_file_copy();
