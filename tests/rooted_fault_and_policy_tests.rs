    ...
    run_in_test_fault_process(TEST_NAME, "rooted-temp-directory-create", || {
        ...
        let error = rooted
            .create_temp_directory_with_options(
                &LocalTempDirectoryOptions::new().with_max_attempts(2),
            )
...
fn test_rooted_copy_rejects_fifo_source() {
    ...
    let native_path =
        CString::new(fifo.as_os_str().as_bytes()).expect("temporary path should not contain NUL");
...
fn test_rooted_local_file_system_rejects_regular_file_anchor() {
    ...
    let error =
        LocalFileSystem::rooted(&file).expect_err("regular files cannot become rooted authorities");
...
fn test_rooted_local_file_system_walker_rolls_back_path_validation_failure() {
    const TEST_NAME: &str =
        "test_rooted_local_file_system_walker_rolls_back_path_validation_failure";
    run_in_test_fault_process(TEST_NAME, "walker-rooted-relative-path", || {
        let directory = tempdir().expect("temporary root should be created");
        let rooted = LocalFileSystem::rooted(directory.path()).expect("root authority should open");
        let mut walker = rooted
...
fn test_rooted_writer_retries_an_injected_staging_collision() {
    ...
    run_rooted_writer_fault(TEST_NAME, "rooted-staging-collision", || {
        ...
        let _ = writer
            .abort()
            .expect("retried staging entry should clean up");
    });
}
