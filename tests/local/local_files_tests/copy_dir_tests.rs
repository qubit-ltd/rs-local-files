...
use super::super::test_support::{CURRENT_DIR_LOCK, CurrentDirGuard, fs, temp_dir};
#[cfg(unix)]
use super::super::test_support::{
    PermissionsExt, assert_fifo_open_is_rejected, create_fifo, short_temp_dir,
};
#[cfg(target_os = "linux")]
use super::super::test_support::{SourceReadLease, current_thread_cpu_time};
...
fn assert_injected_copy_error(
    ...
    let Some(()) = run_in_coverage_fault_process(test_name, fault, move || {
        ...
        if with_file {
            fs::write(src.join("data.txt"), b"source").expect("source file should be written");
        }
...
fn test_copy_dir_rejects_injected_directory_identity_cycle() {
    ...
    let Some(()) =
        run_in_coverage_fault_process(TEST_NAME, "copy-dir-directory-identity-cycle", move || {
            ...
            let error =
                qubit_local_files::copy::directory(&src, &dst, LocalCopyDirOptions::default())
...
fn test_copy_dir_reconciles_injected_existing_directory_race() {
    ...
    let Some(()) = run_in_coverage_fault_process(TEST_NAME, "copy-directory-race-existing", || {
        let dir = temp_dir("copy-directory-race-existing");
        let src = dir.join("src");
        let dst = dir.join("dst");
        fs::create_dir(&src).expect("source directory should be created");

        let stats = qubit_local_files::copy::directory(&src, &dst, LocalCopyDirOptions::default())
...
fn assert_injected_root_type_replacement_error(test_name: &str, fault: &str) {
    let Some(()) = run_in_coverage_fault_process(test_name, fault, move || {
        ...
        let dst = dir.join("dst");
        fs::create_dir(&src).expect("source directory should be created");
        fs::write(&dst, b"destination").expect("destination file should be written");

        let error = qubit_local_files::copy::directory(
            &src,
            &dst,
            LocalCopyDirOptions::new().with_type_conflict(LocalCopyTypeConflictPolicy::Replace),
        )
...
fn test_copy_dir_handles_injected_directory_before_type_replacement() {
    ...
    assert_injected_root_type_replacement_error(TEST_NAME, "copy-removal-race-directory");
}
...
fn test_copy_dir_handles_injected_disappearance_before_type_replacement() {
    ...
    assert_injected_root_type_replacement_error(TEST_NAME, "copy-removal-race-not-found");
}
...
fn test_copy_dir_reports_injected_type_replacement_inspection_error() {
    ...
    assert_injected_root_type_replacement_error(TEST_NAME, "copy-removal-race-inspect");
}
...
fn test_copy_dir_all_with_handles_deep_tree_on_small_stack() {
    ...
    const CHILD_ENVIRONMENT: &str = "QUBIT_LOCAL_FILES_COPY_DIR_SMALL_STACK_CHILD";

    let Some(dir) = run_in_small_stack_process(TEST_NAME, CHILD_ENVIRONMENT, || {
        let dir =
            std::path::PathBuf::from(format!("/tmp/qio-{}-deep-copy-dir", std::process::id(),));
        drop(fs::remove_dir_all(&dir));
        let src = dir.join("src");
        let dst = dir.join("dst");
        fs::create_dir_all(&src).expect("deep copy source should be created");
        let mut current = src.clone();
        let mut relative = std::path::PathBuf::new();
        for _ in 0..512 {
            current.push("d");
            relative.push("d");
            fs::create_dir(&current).expect("deep copy directory should be created");
        }
        fs::write(current.join("leaf"), b"x").expect("deep copy leaf should be written");
...
fn test_small_stack_process_uses_stable_child_current_dir() {
    ...
    const CHILD_ENVIRONMENT: &str = "QUBIT_LOCAL_FILES_STABLE_CHILD_CURRENT_DIR";

    if std::env::var_os(CHILD_ENVIRONMENT).is_some() {
        std::env::current_dir().expect("child current directory should be readable");
        return;
    }
    ...
    let result = run_in_small_stack_process(TEST_NAME, CHILD_ENVIRONMENT, || ());
...
fn test_copy_dir_all_with_binds_relative_paths_before_source_open() {
    ...
    const CHILD_ENVIRONMENT: &str = "QUBIT_LOCAL_FILES_COPY_DIR_RELATIVE_PATH_CHILD";
    ...
    let Some(dir) = run_in_small_stack_process(TEST_NAME, CHILD_ENVIRONMENT, || {
        ...
        fs::write(&source_file, b"creation").expect("source file should be written");

        let _guard = CurrentDirGuard::change_to(&creation_dir);
        let lease =
            SourceReadLease::acquire(&source_file).expect("source read lease should be acquired");
        ...
        std::env::set_current_dir(&later_dir).expect("current directory should change");
        lease
            .release()
...
fn test_copy_dir_all_with_rejects_existing_root_destination_without_overwrite() {
    ...
    let error = qubit_local_files::copy::directory(&src, &dst, LocalCopyDirOptions::default())
...
fn test_copy_dir_all_with_returns_source_entry_metadata_error_without_search_permission() {
    ...
    let error = qubit_local_files::copy::directory(&src, &dst, LocalCopyDirOptions::default())
        .expect_err("source entries should not be inspectable without search permission");
...
fn test_copy_dir_all_with_skips_existing_destination_files() {
    ...
    fs::create_dir(&src).expect("source directory should be created");
    fs::create_dir(&dst).expect("destination directory should be created");
    fs::write(src.join("data.txt"), b"new").expect("source file should be written");
    fs::write(dst.join("data.txt"), b"old").expect("destination fixture should be written");
...
/// panic resumes.
#[cfg(target_os = "linux")]
fn run_copy_after_staging<T, A, C>(source_file: &std::path::Path, action: A, copy: C) -> T
where
    T: Send + 'static,
    A: FnOnce(),
    C: FnOnce() -> T + Send + 'static,
{
    ...
    if let Err(error) = lease.wait_for_break() {
        drop(lease.release());
        drop(worker.join());
        panic!("copy worker should block while opening the leased source: {error}");
    }
...
fn test_copy_dir_zero_open_retry_timeout_reports_timed_out() {
    ...
    let lease =
        SourceReadLease::acquire(&source_file).expect("source read lease should be acquired");
    let options = LocalCopyDirOptions::new().with_open_retry_timeout(Duration::ZERO);
    let worker_source = source.clone();
    let worker_destination = destination.clone();
    let (sender, receiver) = std::sync::mpsc::sync_channel(1);
    let worker = std::thread::spawn(move || {
        let result =
            qubit_local_files::copy::directory(&worker_source, &worker_destination, options);
...
fn test_source_read_lease_retries_transient_conflict() {
    ...
    let lease = SourceReadLease::acquire_with_lease_operation(&source_file, |file| {
...
fn test_copy_dir_all_with_waits_for_source_lease_without_busy_polling() {
    ...
    let (result, worker_cpu_time) = run_copy_after_staging(
        ...
        move || {
            let started =
                current_thread_cpu_time().expect("worker CPU start time should be readable");
            ...
            let finished =
                current_thread_cpu_time().expect("worker CPU finish time should be readable");
    ...
    let result = run_copy_after_staging(
        ...
        move || {
            qubit_local_files::copy::directory(
                &copy_src,
                &copy_dst,
                LocalCopyDirOptions::new().with_open_retry_timeout(OPEN_RETRY_TIMEOUT),
            )
        },
    );
...
#[cfg(target_os = "linux")]
pub(super) fn directory_write_restrictions_are_enforced(path: &std::path::Path) -> bool {
...
fn test_copy_dir_all_with_reports_staging_cleanup_failure() {
    ...
    let error = run_copy_after_staging(
        &source_file,
        move || {
            fs::set_permissions(&restricted_dst, fs::Permissions::from_mode(0o500))
                .expect("destination write permission should be removed");
        },
        move || {
            qubit_local_files::copy::directory(&copy_src, &copy_dst, LocalCopyDirOptions::default())
        },
    )
...
fn test_copy_dir_all_with_reports_skipped_staging_cleanup_failure() {
    ...
    let destination_contents =
        fs::read(&destination_file).expect("racing destination should remain readable");
    fs::remove_dir_all(dir).expect("test directory should be removed");
...
fn test_copy_dir_all_with_handles_destination_created_after_staging() {
    ...
    fs::create_dir(&skip_dst).expect("skip destination directory should be created");
    let copy_src = src.clone();
    let copy_dst = skip_dst.clone();
    let stats = run_copy_after_staging(
        &source_file,
        || {
            fs::write(&skip_target, b"raced").expect("racing skip destination should be written");
        },
        move || {
            qubit_local_files::copy::directory(
                &copy_src,
    ...
    fs::create_dir(&fail_dst).expect("fail destination directory should be created");
    let copy_src = src.clone();
    let copy_dst = fail_dst.clone();
    let error = run_copy_after_staging(
        &source_file,
        || {
            fs::write(&fail_target, b"raced").expect("racing fail destination should be written");
        },
        move || {
            qubit_local_files::copy::directory(&copy_src, &copy_dst, LocalCopyDirOptions::default())
        },
    )
    .expect_err("destination created after staging should fail conservative copy");
...
fn test_copy_dir_all_with_preserves_file_replacing_directory_after_staging() {
    ...
    let stats = run_copy_after_staging(
        ...
        move || {
            ...
            fs::write(&racing_entry, b"raced").expect("racing destination file should be written");
        },
        move || {
            qubit_local_files::copy::directory(
                &copy_src,
...
fn test_copy_dir_all_with_reports_directory_removal_error_after_staging() {
    ...
    fs::create_dir_all(&destination_entry)
        .expect("conflicting destination directory should be created");
    fs::write(&marker, b"keep").expect("destination marker should be written");
    fs::create_dir(&permission_probe).expect("permission probe should be created");
    ...
    fs::set_permissions(&dst, fs::Permissions::from_mode(0o700))
        .expect("destination permissions should be restored");
    let error = result.expect_err("non-writable destination should reject directory removal");
...
fn test_copy_dir_all_with_reports_reinspection_error_after_staging() {
    ...
    let error = run_copy_after_staging(
        &source_file,
        move || {
            fs::set_permissions(&restricted_dst, fs::Permissions::from_mode(0o400))
                .expect("destination search permission should be removed");
        },
        move || {
            qubit_local_files::copy::directory(
...
fn test_copy_dir_all_with_returns_destination_entry_inspection_error() {
    ...
    let dst = dir.join("dst");
    fs::create_dir(&src).expect("source directory should be created");
    fs::write(src.join("data.txt"), b"data").expect("source file should be written");
    fs::create_dir(&dst).expect("destination directory should be created");
...
fn test_copy_dir_all_with_returns_nested_destination_inspection_error() {
    ...
    let src = dir.join("src");
    let dst = dir.join("dst");
    fs::create_dir_all(src.join("nested")).expect("nested source directory should be created");
    fs::create_dir(&dst).expect("destination directory should be created");
    fs::set_permissions(&dst, fs::Permissions::from_mode(0o600))
        .expect("destination search permission should be removed");
...
fn test_copy_dir_all_with_returns_destination_removal_permission_error() {
    ...
    fs::write(&dst, b"existing").expect("conflicting destination file should be written");
...
fn test_copy_dir_all_with_preserves_root_file_when_type_replacement_removal_fails() {
    ...
    fs::create_dir(&src).expect("source directory should be created");
    fs::create_dir(&protected_parent).expect("destination parent should be created");
...
fn test_copy_dir_all_with_directory_symlink_options() {
    ...
    let error = qubit_local_files::copy::directory(&src_link, &dst, LocalCopyDirOptions::default())
...
fn test_copy_dir_all_with_does_not_preserve_permissions_by_default() {
    ...
    fs::set_permissions(src.join("data.txt"), fs::Permissions::from_mode(0o644))
        .expect("source file permissions should be set");

    let _ = qubit_local_files::copy::directory(&src, &dst, LocalCopyDirOptions::default())
