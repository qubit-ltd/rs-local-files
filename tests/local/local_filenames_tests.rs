...
proptest! {
    ...
    fn test_file_name_from_url_handles_arbitrary_utf8(url in any::<String>()) {
        let _ = path::file_name_from_url(&url);
    }
    ...
    fn test_file_name_from_url_keeps_generated_unsafe_fragments(
        ...
        prop_assert_eq!(raw_name, path::file_name_from_url(&url));
    }
    ...
    fn test_file_name_from_url_keeps_generated_dot_components(
        ...
        prop_assert_eq!(encoded, path::file_name_from_url(&url));
    }
    ...
    fn test_validate_portable_generated_acceptance_implies_invariants(
        ...
        if path::validate_portable_file_name(&name).is_ok() {
...
fn test_validate_portable_file_name_accepts_safe_names() {
    for name in [
        ...
        path::validate_portable_file_name(name)
...
fn test_file_name_returns_final_component() {
    ...
    assert_eq!(Some("archive.tar.gz"), path::file_name(path));
    assert_eq!(None, path::file_name(Path::new("/")));
}
...
fn test_extension_helpers_handle_missing_and_empty_extensions() {
    assert_eq!(None, path::extension(Path::new("README")));
    assert_eq!(None, path::dot_extension(Path::new("README")));
    assert_eq!(Some(""), path::extension(Path::new("name.")));
    assert_eq!(Some(String::new()), path::dot_extension(Path::new("name.")));
}

#[test]
fn test_dotfiles_follow_rust_path_semantics() {
    assert_eq!(Some(".env"), path::file_stem(Path::new(".env")));
    assert_eq!(None, path::extension(Path::new(".env")));

    assert_eq!(Some(".config"), path::file_stem(Path::new(".config.toml")));
    assert_eq!(Some("toml"), path::extension(Path::new(".config.toml")));
}
...
fn test_has_extension_accepts_optional_leading_dot() {
    ...
    assert!(path::has_extension(path, "PDF"));
    assert!(path::has_extension(path, ".PDF"));
    assert!(!path::has_extension(path, "pdf"));
    assert!(path::has_extension_ignore_ascii_case(path, "pdf"));
    assert!(path::has_extension_ignore_ascii_case(path, ".pdf"));
}

#[test]
fn test_file_name_from_path_handles_common_separators() {
    assert_eq!("file.txt", path::file_name_from_path("/tmp/data/file.txt"));
    assert_eq!(
        "file.txt",
        path::file_name_from_path(r"C:\tmp\data\file.txt")
    );
    assert_eq!("file.txt", path::file_name_from_path("file.txt"));
    assert_eq!("", path::file_name_from_path("/tmp/data/"));
}
...
fn test_file_name_from_url_removes_query_and_fragment() {
    ...
    assert_eq!(
        "file.txt",
        path::file_name_from_url("https://example.com/path/file.txt#section")
    );
    assert_eq!(
        "file.txt",
...
fn test_file_name_from_url_excludes_url_authority() {
    for url in [
        ...
        assert_eq!("", path::file_name_from_url(url));
    }
}
...
fn test_file_name_from_url_supports_hierarchical_and_opaque_urls() {
    for (url, expected) in [
        ...
        assert_eq!(expected, path::file_name_from_url(url));
    }
}
...
fn test_file_name_from_url_decodes_percent_encoded_utf8() {
    assert_eq!(
        "my file.txt",
        path::file_name_from_url("https://example.com/path/my%20file.txt")
    );
    assert_eq!(
        format!("caf{}.txt", '\u{00e9}'),
        path::file_name_from_url("https://example.com/path/caf%C3%A9.txt")
    );
    assert_eq!(
        "file+plus.txt",
        path::file_name_from_url("https://example.com/path/file%2Bplus.txt")
    );
}
...
fn test_file_name_from_url_keeps_invalid_percent_encoding() {
    assert_eq!(
        "file%ZZ.txt",
        path::file_name_from_url("https://example.com/path/file%ZZ.txt")
    );
    assert_eq!(
        "file%2.txt",
        path::file_name_from_url("https://example.com/path/file%2.txt")
    );
}

#[test]
fn test_filenames_random_with_uses_prefix_suffix_pid_and_hex_payload() {
    let name = path::random_file_name_with(Some("pre-"), Some(".suf"))
        .expect("random name should be generated");
    let body = name
        .strip_prefix("pre-")
...
fn test_filenames_try_random_with_rejects_path_fragments() {
    let error = path::random_file_name_with(Some("../escape-"), None)
        .expect_err("prefix with path separators should be rejected");
    assert_eq!(ErrorKind::InvalidInput, error.kind());

    let error = path::random_file_name_with(None, Some("/suffix"))
        .expect_err("suffix with path separators should be rejected");
    assert_eq!(ErrorKind::InvalidInput, error.kind());

    let error = path::random_file_name_with(Some("bad\0prefix"), None)
        .expect_err("prefix with NUL bytes should be rejected");
    assert_eq!(ErrorKind::InvalidInput, error.kind());

    let error = path::random_file_name_with(Some(".."), None)
        .expect_err("parent directory component should be rejected");
    assert_eq!(ErrorKind::InvalidInput, error.kind());

    let name = path::random_file_name_with(Some("safe-"), Some(".tmp"))
