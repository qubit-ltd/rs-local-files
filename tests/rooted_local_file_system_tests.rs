...
/// Verifies cleanup follows the root descriptor after its diagnostic path moves.
#[test]
...
/// Verifies directory cleanup follows retained root authority after replacement.
#[test]
...
/// Verifies rooted persistence keeps using the opened root after diagnostic rename.
#[test]
...
/// Verifies an indeterminate native persist failure disables cleanup and drop removal.
#[test]
...
fn test_rooted_copy_failure_reports_second_child_partial_publication() {
    const TEST_NAME: &str =
        concat!("test_rooted_copy_failure_reports_second_child_partial_publication");
...
fn test_rooted_copy_failure_retains_stats_after_parent_sync_fault() {
    const TEST_NAME: &str =
        concat!("test_rooted_copy_failure_retains_stats_after_parent_sync_fault");
...
/// Verifies rooted commit cleanup failures retain root-relative staging details.
#[cfg(coverage)]
#[test]
fn test_rooted_copy_failure_retains_cleanup_staging_context() {
    const TEST_NAME: &str = concat!("test_rooted_copy_failure_retains_cleanup_staging_context");
...
fn test_rooted_rename_parent_durability_failure_reports_renamed() {
    const TEST_NAME: &str = concat!("test_rooted_rename_parent_durability_failure_reports_renamed");
...
fn test_rooted_rename_native_io_failure_reports_indeterminate() {
    const TEST_NAME: &str = concat!("test_rooted_rename_native_io_failure_reports_indeterminate");
