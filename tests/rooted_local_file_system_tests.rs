...
use qubit_local_files::{
    LocalCopyConflictPolicy, LocalCopyOptions, LocalCopyTypeConflictPolicy,
    LocalCreateDirectoryOptions, LocalDeleteOptions, LocalFileErrorKind, LocalFileKind,
    LocalFileSystem, LocalListOptions, LocalPersistFailureState, LocalPersistStage,
    LocalReadOptions, LocalRenameFailureState, LocalRenameOptions, LocalTempDirectoryOptions,
    LocalTempFileOptions, LocalWriteMode, LocalWriteOptions,
};
#[cfg(feature = "internal-test-support")]
use qubit_local_files::{LocalCopyFailureState, LocalDurabilityRequirement};
...
fn test_copy_tree_skips_nested_type_conflicts() {
    ...
    fs::write(
        directory.path().join("source/file-to-directory"),
        b"source-file",
    )
    .expect("source file should be written");
    fs::write(
        directory.path().join("source/directory-to-file/hidden"),
        b"hidden",
    )
    ...
    fs::write(
        directory.path().join("target/file-to-directory/kept"),
        b"kept",
    )
    .expect("target child should be written");
    fs::write(
        directory.path().join("target/directory-to-file"),
        b"target-file",
    )
...
/// Runs one test-support-only rooted-copy fault case in an isolated child process.
#[cfg(feature = "internal-test-support")]
...
fn test_rooted_local_file_system_walker_rejects_handle_budget_exhaustion() {
    let directory = tempdir().expect("temporary root should be created");
    fs::create_dir(directory.path().join("nested")).expect("nested directory should be created");
    fs::write(directory.path().join("nested/child"), b"payload")
    ...
    let mut walker = rooted
        .list(
            ...
            &LocalListOptions::new()
                ...
                .with_reopen_policy(qubit_local_files::LocalDirectoryReopenPolicy::Fail),
        )
...
fn test_rooted_local_file_system_walker_reopens_handle_frames() {
    ...
    let entries = rooted
        .list(
            ...
            &LocalListOptions::new()
                .with_recursive()
                .with_max_open_directories(1)
                .with_reopen_policy(qubit_local_files::LocalDirectoryReopenPolicy::Reopen),
        )
        .expect("rooted walker should open")
        .collect::<Result<Vec<_>, _>>()
    ...
    assert!(entries.iter().any(|entry| {
        entry.relative_path() == Path::new("level-0/level-1/level-2/level-3/payload")
    }));
}
