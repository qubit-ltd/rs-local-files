...
fn test_local_file_system_copy_symlink_policy_matrix() {
    ...
    assert_eq!(
        ...
        fs::read_link(&overwrite_target).unwrap()
    );
    ...
    assert_eq!(
        PathBuf::from("referent"),
        fs::read_link(&directory_target).unwrap()
    );
...
fn test_recursive_copy_symlink_destination_policy_matrix() {
    ...
    fs::write(overwrite_target.join("link"), b"existing")
        .expect("overwrite fixture should be written");
    let overwritten = LocalFileSystem::host()
        .expect("Host filesystem should open")
        .copy_with_options(
            &source,
    ...
    let directory_target = directory.path().join("directory-target");
    fs::create_dir_all(directory_target.join("link"))
        .expect("nested directory target should be created");
    LocalFileSystem::host()
        .expect("Host filesystem should open")
        .copy_with_options(
            &source,
