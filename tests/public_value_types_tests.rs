...
fn test_capability_snapshot_exposes_all_guarantees() {
    ...
    let atomic_replace = std::hint::black_box(
        LocalFileSystemProtocols::supports_atomic_replace as fn(LocalFileSystemProtocols) -> bool,
    );
    assert_eq!(
...
fn test_local_paths_cover_public_composition_and_binding_cases() {
    ...
    assert!(
        LocalPaths::to_canonical_components(LocalFileSystemScope::Rooted, Path::new(""),).is_ok()
    );
}
