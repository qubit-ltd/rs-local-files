...
fn test_copy_failure_reports_second_child_partial_publication() {
    const TEST_NAME: &str = concat!("test_copy_failure_reports_second_child_partial_publication");
    run_in_coverage_fault_process(TEST_NAME, "copy-staging-copy-second", || {
...
fn test_copy_failure_reports_published_after_parent_sync_fault() {
    const TEST_NAME: &str = concat!("test_copy_failure_reports_published_after_parent_sync_fault");
...
fn test_copy_failure_retains_staging_only_for_cleanup_failure() {
    const TEST_NAME: &str = concat!("test_copy_failure_retains_staging_only_for_cleanup_failure");
    run_in_coverage_fault_process(TEST_NAME, "copy-staging-copy-cleanup", || {
        let directory = tempfile::tempdir().expect("temporary directory should be created");
        let source = directory.path().join("source");
        let target = directory.path().join("target");
        fs::write(&source, b"payload").expect("source file should be written");
...
fn test_copy_failure_omits_staging_after_successful_cleanup() {
    const TEST_NAME: &str = concat!("test_copy_failure_omits_staging_after_successful_cleanup");
...
/// Verifies destination preparation errors without publication are conservative.
#[cfg(coverage)]
#[test]
fn test_copy_failure_reports_indeterminate_for_destination_preparation_fault() {
    const TEST_NAME: &str =
        concat!("test_copy_failure_reports_indeterminate_for_destination_preparation_fault");
    run_in_coverage_fault_process(TEST_NAME, "copy-destination-absolute", || {
        let directory = tempfile::tempdir().expect("temporary directory should be created");
        let source = directory.path().join("source");
        let target = directory.path().join("target");
        fs::create_dir(&source).expect("source directory should be created");
