...
fn test_local_directory_walker_non_recursive_listing_retains_bound_root() {
    ...
    assert_eq!(bound_path(&directory.path().join("top")), entries[1].diagnostic_path(),);
}
...
fn test_local_directory_walker_zero_max_depth_yields_no_entries() {
    let directory = tempdir().expect("temporary directory should be created");
    fs::write(directory.path().join("entry"), b"payload").expect("entry fixture should be written");
...
fn test_local_directory_walker_resource_limit_terminates_continue_policy() {
    let directory = tempdir().expect("temporary directory should be created");
    fs::write(directory.path().join("first"), b"first").expect("first entry should be written");
    fs::write(directory.path().join("second"), b"second").expect("second entry should be written");
...
fn test_local_directory_walker_rejects_unrepresentable_deadline() {
    ...
    let error = LocalFileSystem::host()
        .list(directory.path(), &LocalListOptions::new().with_deadline(Duration::MAX))
...
fn test_local_directory_walker_rejects_zero_open_directory_budget() {
    ...
    let error = LocalFileSystem::host()
        .list(directory.path(), &LocalListOptions::new().with_max_open_directories(0))
...
fn test_local_directory_walker_follow_mode_traverses_links_and_rejects_cycles() {
    ...
    fs::create_dir(&target).expect("outside target directory should be created");
    fs::write(target.join("child"), b"payload").expect("target child should be written");
    symlink(&target, directory.path().join("link")).expect("directory link should be created");
...
fn test_local_directory_walker_reports_unreadable_child_directory() {
    ...
    let error = result.expect_err("unreadable child descent must return an error");
...
fn test_local_directory_walker_reports_injected_iteration_failures() {
    ...
    for fault in [
        ...
        run_walker_fault_process(TEST_NAME, fault, || {
            ...
            let result = LocalFileSystem::host().list(directory.path(), &options);
