...
use qubit_local_files::{LocalFileKind, LocalFileSystem, LocalListOptions};
...
fn test_local_directory_walker_recurses_lazily() {
    ...
    let walker = LocalFileSystem::list(directory.path(), &LocalListOptions::new().with_recursive())
...
fn test_local_directory_walker_does_not_follow_symlinks_by_default() {
    ...
    let entries = LocalFileSystem::list(directory.path(), &LocalListOptions::new())
        .expect("walker should be created")
        .collect::<Result<Vec<_>, _>>()
