...
use qubit_local_files::{atomic, directory, metadata, remove, rename};
...
fn test_native_modules_cover_common_file_lifecycle() {
    let directory_handle = tempdir().expect("a temporary directory should be created");
    ...
    let mut writer = atomic::begin(&source).expect("an atomic writer should be created");
    writer
        .write_all(b"payload")
    ...
    assert!(metadata::exists(&source).expect("source existence should be observable"));
    assert_eq!(
        7,
    ...
    rename::move_path(&target, &moved).expect("the copied file should be renamed");
